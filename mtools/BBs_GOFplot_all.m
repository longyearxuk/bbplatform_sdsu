...
EVENT	= 'Ridgecrestc';
RLZdir	= '/BBout_0816';	% path to compared rlzs
EVEpath	= '../sngl_rlz/ridgecrest19c';
%
TITLES = [EVENT ' -fac- tests'];
printname = [EVENT '_fac_test'];
%
rlzs = 0;		% rlz start number
rlzc = 6;		% rlz counts for comparison
rlzn = {'fac=1.4' 'fac=1.6' 'fac=1.7' 'fac=1.8' 'fac=1.9' 'fac=2.0'};		% labels for different tests
...
% initiate the figures for plot
figure(1);
picH = 200*rlzc+10;
set(gcf,'OuterPosition',[0,0,600,picH]);
set(gcf,'color','white');
maxX = 10.0;
minX = 0.01;
   
% --------------------------------------- %   
% iterations through compared rlzs
% --------------------------------------- %   
...
% count total stations at each period
  nsta_inSA = ones(leng,nsta);	% record stats at each T
  for ss=1:nsta
    for mm=1:leng

      % eliminate Inf in residuals
      if abs(resSA(mm,ss)) == Inf
        resSA(mm,ss) = 0;
      end
  
      % eliminate out-limit periods (by setting inSA=0)
  ...
  Tmax_nsta = sum(nsta_inSA');	% total stations at each T
...
% calculate standard error & 90% range for each rlz
% use Rob's approach for standard error calculations

  resSD = zeros(leng,1);
  res90 = zeros(leng,1);
  % summed squares for residuals (all stations)
  sq_resSA = sum((resSA.^2)')';
  sq_resMD = Tmax_nsta.*(resMD.^2);

  for mm=1:leng
    n = Tmax_nsta(mm);

    % RMS and 90%-conf intvs for periods with >1 stations
    if n > 1
      resSD(mm,1) = sqrt((sq_resSA(mm)-sq_resMD(mm))/(n-1));
      res90(mm,1) = 1.699*sqrt(1/n)*resSD(mm,1);   
    end
  end

  % upper and lower ranges for SD & 90%-conf
  SD_ur = resMD+resSD; c90_ur = resMD+res90;
  SD_lr = resMD-resSD; c90_lr = resMD-res90;

% --------------------------------------- %
% plot GOF for each realization

  picC = subplot(rlzc, 1, ii-rlzs+1);
  pp=patch([(T)' fliplr((T'))],[SD_ur' fliplr(SD_lr')],[0.8 0.8 0.8]);
  ...
  axis tight;
  %set(picC,'Position', [0.08, 0.08, 0.88, 0.85]);
  set(picC,'LineW',2,'XTick',[0.01 0.05 0.1 0.2 0.3 0.5 1 1.5 2 3 4 5 7.5 10],...
  ...
  text(minX+0.005, 1.3, ['RLZ #' ii_str ': ' rlzn{ii+rlzs+1}],'Fontsize',12,'FontW','bo');
  grid on

  if ii == rlzs
    ll=legend('1-\sigma interval','90 % C.I.','median','BBs');
    set(ll,'FontS',8,'box','off', 'FontW', 'bo', 'Location', 'Southwest');
    title(['RotD50 plots for ' TITLES]);
  end
% --------------------------------------- %
  tocc=toc;
